...
int main(int argc, char *argv[]) {
  ...
  int connectionNums[4] = {0, 0, 0, 0};
  int portNums[4] = {5001, 5002, 5003, 5004};
	while(cnt < 5){
    if(cnt < 5) {
      printf("Request ID: %d\n", cnt++);
      ...
      // 임의로 두 개의 서버 선택
      int randomPort1 = rand() % 4;
      int randomPort2 = rand() % 4;
      while(randomPort1 == randomPort2) {
        randomPort2 = rand() % 4;
      }

      // 선택된 두 개의 서버 중에서 누적 요청의 수 적은 서버 선택
      int selectedPort = (connectionNums[randomPort1] < connectionNums[randomPort2]) ? randomPort1 : randomPort2;
      SERVER_PORT = portNums[selectedPort];
      srv_addr.sin_port = htons(SERVER_PORT);
      sendto(sock, &SendMsg, pkt_size, 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr)); // 생성한 메시지를 서버로 송신한다.

      connectionNums[selectedPort] += 1; // 요청 보낸 서버의 전송한 누적 요청 수 증가
    }    
    ...
    while(1) {
      ...
      if (n > 0) { // 만약 송신한 데이터가 0바이트를 초과한다면 (즉, 1바이트라도 수신했다면)
        if (RecvMsg.type == READ_REP) {
          ...
          printf("Type: READ_REP Key: %s Value: %s\n\n",RecvMsg.key, RecvMsg.value); // 수신한 내용을 출력한다.
          break;
        }
      }
    }  
