...
int main(int argc, char *argv[]) {
  ...
  size_t pkt_size = 0;
    ...
    sendto(sock, &SendMsg, pkt_size, 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr)); // 생성한 메시지를 캐시 서버로 송신한다.
    ...
    n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, &src_addr_len); // 서버로부터 답장을 수신한다.
      ...
      if (RecvMsg.type == READ_REP) { // Cache Hit 가 발생했다면
        printf("Cache hit!!\n");
        printf("Received bytes: %d\n",n);
        printf("Type: %s Key: %s Value: %s\n\n",get_type(RecvMsg),RecvMsg.key, RecvMsg.value); // 수신한 내용을 출력한다.
      }

      else { // Cache Miss 가 발생했다면

        // 스토리지 서버에 읽기 요청을 보내 key에 대한 value 값을 읽어온다.
